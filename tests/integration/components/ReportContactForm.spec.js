...
describe("Report Contact Form", () => {
  ...
  describe("Form submition", () => {
    it("should display alert if no option selected", async () => {
      ...
      const objectInput = wrapper.find("input[name=\"object\"]");
      ...
      expect(alert.text()).toBe(t("report-contact-form.alert.select-option"));
    });
    ...
    it("should hide form and display success message if form is submited", async () => {
      ...
      expect(successMessage.text()).toBe(t("report-contact-form.success-message"));
    });
  });
});
