  ...
  <div class="max-w-4xl mx-auto p-4">
    ...
    <article class="space-y-6">
      <div v-if="versionInformation" class="border border-gray-200 p-4 rounded-lg">
        <h3 class="text-xl font-medium mb-2">{{ $t('desktop.mac') }}</h3>
        <a
            :href="versionInformation.assets[0].browser_download_url"
            class="text-blue-600 hover:underline"
        >
          {{ $t("desktop.downloadMac", { version: versionInformation.tag_name }) }}
        </a>
      </div>

      <div v-if="versionInformation" class="border border-gray-200 p-4 rounded-lg">
        <h3 class="text-xl font-medium mb-2">{{ $t('desktop.windows') }}</h3>
        <a
            :href="versionInformation.assets[1].browser_download_url"
            class="text-blue-600 hover:underline"
        >
          {{ $t("desktop.downloadWindows", { version: versionInformation.tag_name }) }}
        </a>
      </div>
