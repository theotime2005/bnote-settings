...
-
    - number: A number.
- default(required): The default value of the parameter that is retrieved
  in [the B.note project](https://github.com/devel-erb/bnote),
- min(optional): The minimum value (only for numbers),
- max(optional): The maximum value (only for numbers),
  -isTranslate(optional): Tells the component if it should use translations when it comes to a menu for example and
  items should not be translated.
...
- Then modify [the management page](../src/views/SettingsView.vue)

```vue

<div id="category" :aria-label="$t('settings.id.category')" class="space-y-4">
  <h3 class="text-xl font-semibold text-gray-700">{{ $t("settings.id.category") }}</h3>
  <button
    Type="button"
    @Click="togle_menu('category')"
    Class="text-sm text-blue-500 hover:underline"
  >
    {{ display_menu["category"]? $t("settings.page.hide"): $t("settings.page.show") }}
  </button>
  <div class="setting" v-if="display_menu['category']">
    <SettingComponent
      V-for="setting in all_settings['category']"
      :Key="setting['id']"
      :Setting="setting"
      :setting_value="settingsData['category'][setting['id']]"
      :Name="$t(`settings.id.${setting['id']}`)"
      :Label_id="`category.${setting['id']}`"
      @Setting-change="save_new_value('category', setting['id'], $event)"
    />
  </div>
</div>
```

In this example, replace category with the name of the category of your parameter. Add the corresponding key in "
display_menu".
